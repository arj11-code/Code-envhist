...
# write to CSV
with open("trajectory_100.csv", "w", newline="") as fcsv:
    ...
    for t, community in enumerate(community_list):
        for i in range(nspecies):
            if community[i] > 1e-5:  # optional: keep only surviving species
...
plt.tight_layout()
plt.savefig("community_root_fr_100.svg", format="svg", bbox_inches="tight")
plt.show()
...
ax.set_ylabel("Abundance")
plt.tight_layout()
plt.savefig("community_root_phi_100.svg", format="svg", bbox_inches="tight")
plt.show()
'''
